...
export const Route = createFileRoute('/drinques/shot')({
  component: Shots,
  validateSearch: z.object({ pagina: z.number().catch(1) }),
  loaderDeps: ({ search }) => ({ page: search.pagina }),
  loader: async ({ deps }) =>
    await getDrinks({ data: { category: 'shot', page: deps.page } }),
  head: () => ({ meta: [{ title: 'Shots - Drinquepedia' }] }),
});

function Shots() {
  const data = Route.useLoaderData();
  const search = useSearch({ from: '/drinques/shot' });
